#!/usr/bin/env bash
# Build a macOS .pkg installer for polez-vst (AU v2 + CLAP + VST3) from the current build tree.
# Run from repo root after: cargo truce build --clap --vst3 --au2
#
# Usage:
...
# Install location:
#   /Library/Audio/Plug-Ins/Components/Polez.component
...
PLUGIN_AU_NAME="Polez.component"
...
AU_PATH="${REPO_ROOT}/target/bundles/${PLUGIN_AU_NAME}"
CLAP_PATH="${REPO_ROOT}/target/bundles/${PLUGIN_CLAP_NAME}"
VST3_PATH="${REPO_ROOT}/target/bundles/${PLUGIN_VST3_NAME}"

if [ ! -d "$AU_PATH" ] || [ ! -d "$CLAP_PATH" ] || [ ! -d "$VST3_PATH" ]; then
  echo "Error: Plugin bundles not found. Build first:" >&2
  echo "  cargo truce build --clap --vst3 --au2" >&2
  find "${REPO_ROOT}/target/bundles" -type d \( \
    -name "*.component" -o -name "*.clap" -o -name "*.vst3" \
    \) 2>/dev/null || true
  exit 1
fi
...
rm -rf \
  ...
  "release-artefacts/${PLUGIN_CLAP_NAME}" \
